...
const verifyElevenLabsSignature = ({ signatureHeader, rawBody, secret }) => {
  ...
  const valid =
    isSafeEqualHex(expectedWithTimestamp, signatureV0) ||
    isSafeEqualHex(expectedBodyOnly, signatureV0);
...
const resolveFinalContactStatus = ({
  callSuccessful,
  failureReason,
  callDuration,
  transcriptSummary,
...
const extractWebhookData = (payload = {}) => {
  ...
  const customLlmData =
    ...
    (metadata.dynamic_variables &&
      typeof metadata.dynamic_variables === 'object' &&
      metadata.dynamic_variables) ||
    (data.dynamic_variables &&
      typeof data.dynamic_variables === 'object' &&
      data.dynamic_variables) ||
    {};
...
const handleElevenLabsWebhook = async (req, res, next) => {
  try {
    ...
    if (!verified && allowUnverified) {
      logger.warn(
        '[CampaignWebhook] Processing webhook without valid signature (dev override enabled)',
        {
          reason: verificationReason,
          configuredSecrets: webhookSecrets.length,
        }
    ...
    logger.info(
      '[CampaignWebhook] Duration field scan..........................................................................................................................................................................................................................................................',
      {
    ...
    if (contact.webhookReceivedAt) {
      ...
      if (contact.conversationId === webhookData.conversationId) {
        ...
        if (
          webhookData.callDuration != null &&
          (contact.callDuration == null || contact.callDuration === 0)
        ) {
          idempotentUpdates.callDuration = webhookData.callDuration;
          logger.info(
            '[CampaignWebhook] Updating callDuration from post_call webhook (idempotent path)',
            {
        ...
        if (Object.keys(idempotentUpdates).length > 0) {
          ...
          setImmediate(() => {
            recalculateCampaignMetrics(contact.campaignId).catch((error) => {
              logger.error(
                '[CampaignWebhook] Failed to recalculate campaign metrics after idempotent update',
                {
                  campaignId: contact.campaignId,
                  contactId: contact.id,
                  error: error.message,
      ...
      if (!contact.conversationId && webhookData.conversationId) {
        ...
        if (Object.keys(lateUpdates).length > 0) {
          ...
          setImmediate(() => {
            recalculateCampaignMetrics(contact.campaignId).catch((error) => {
              logger.error(
                '[CampaignWebhook] Failed to recalculate metrics after late conversationId update',
                {
                  campaignId: contact.campaignId,
                  error: error.message,
                }
      ...
      logger.warn(
        '[CampaignWebhook] Webhook received for contact with previous webhook but different conversationId',
        {
          campaignId: contact.campaignId,
          contactId: contact.id,
    ...
    if (webhookData.eventType === 'call_initiation_failure') {
      ...
      setImmediate(() => {
        recalculateCampaignMetrics(contact.campaignId).catch((error) => {
          logger.error(
            '[CampaignWebhook] Failed to recalculate campaign metrics after call initiation failure',
            {
              campaignId: contact.campaignId,
              error: error.message,
            }
    ...
    if (status === 'FAILED') {
      updateData.errorReason =
        ...
        webhookData.transcriptSummary ||
        'Call completed without success';
    } else {

...
async function handleCallResult(req, res, next) {
  try {
    ...
    const enrichmentData = {
      ...
      gatekeeperInfo:
        gatekeeperInfo && Object.keys(gatekeeperInfo).length > 0 ? gatekeeperInfo : null,
    ...
    if (enrichmentStatus === 'identified' && decisionMaker.name) {
      ...
      logger.info(
        `[SDR Level] Establecimiento ${establishmentId} promovido a PROSPECT (tomador decisiones identificado)`
      );
    }
    ...
    if (enrichmentData.level === 'CONTACT' && userId) {
      enqueueSync({
        ...
        logger.warn('[SDR] Error encolando sync con Twenty para CONTACT (no critico)', {
          error: err.message,
        });
      });
    ...
    if (enrichmentData.level === 'PROSPECT' && userId) {
      try {
        ...
        if (leadProspect) {
          ...
          logger.info(
            `[SDR] lead_prospect actualizado para ${establishmentId}, asignado a ${userId}`
          );
        } else {
    ...
    if (testContactId) {
      try {
        ...
        logger.error(
          `[SDR] Error actualizando abTestContact ${testContactId}:`,
          abTestError.message
        );

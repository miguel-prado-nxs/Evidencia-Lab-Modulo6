...
async function exportToPDF(data, columns, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Header
    doc.fillColor('#FF8C00').fontSize(20).text('EasyOrder Partners', { align: 'center' });
    doc
      .fillColor('#666')
      .fontSize(14)
      .text(options.title || 'Reporte', { align: 'center' });
    doc.fontSize(10).text(`Generado: ${new Date().toLocaleDateString('es-MX')}`, {
      align: 'center',
    });
    doc.moveDown(2);
    ...
    doc
      .fillColor('#999')
      .fontSize(8)
      .text(`Total de registros: ${data.length}`, doc.page.margins.left, doc.page.height - 50, {
        align: 'center',
      });
...
const COMMISSIONS_COLUMNS = [
  ...
  { header: 'Estado', key: 'status', accessor: (r) => r.status, width: 12 },
  {
    header: 'Negocio',
    key: 'businessName',
    accessor: (r) => r.deal?.businessName || '',
    width: 25,
  },
  {
...
async function generatePartnerReport(partnerId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.text(
      `Comisiones Totales: $${Number(commissionStats._sum.amount || 0).toLocaleString('es-MX')}`
    );
    doc.moveDown(2);
    ...
    doc
      .fillColor('#999')
      .fontSize(8)
      .text(`Generado el ${new Date().toLocaleString('es-MX')}`, 40, doc.page.height - 50, {
        align: 'center',
      });

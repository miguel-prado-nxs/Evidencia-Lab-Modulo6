  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        .fontSize(16)
        .fillColor('#FF8C00')
        .text('EASYORDER PARTNERS', centerX - 100, 70, {
          width: 200,
          align: 'center',
        });

      // Título principal
      doc.fontSize(40).fillColor('#333').font('Helvetica-Bold').text('CERTIFICADO', 0, 110, {
        width: pageWidth,
        align: 'center',
      });

      doc.fontSize(24).fillColor('#666').font('Helvetica').text('DE FINALIZACIÓN', 0, 155, {
        width: pageWidth,
        align: 'center',
      });
      ...
      // Texto principal
      doc.fontSize(14).fillColor('#666').text('Este certificado se otorga a', 0, 220, {
        width: pageWidth,
        align: 'center',
      });

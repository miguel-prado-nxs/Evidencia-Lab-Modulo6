...
router.get(
  '/ventas/restaurant-profiles/:establishmentId',
  authenticateJWTOrServiceKey,
  restaurantProfileController.getProfile
);

router.post(
  '/ventas/restaurant-profiles',
  authenticateJWTOrServiceKey,
  uploadMemory.single('logo'),
  restaurantProfileController.upsertProfile
);

// PASO 2: SUCURSALES
router.get(
  '/ventas/restaurantes/:restauranteId/sucursales',
  authenticateJWTOrServiceKey,
  async (req, res) => {
    try {
      const { data } = await axios.get(
        `${POS_API_BASE_URL}/api/sucursales/v1/restaurante/${req.params.restauranteId}`,
...
router.put('/ventas/sucursales/:id', authenticateJWTOrServiceKey, async (req, res) => {
  try {
    const { data } = await axios.put(
      `${POS_API_BASE_URL}/api/sucursales/v1/${req.params.id}`,
      req.body,
      { headers: { Authorization: `Bearer ${POS_API_TOKEN}`, 'Content-Type': 'application/json' } }
    );
    res.json(data);
...
// PASO 2: SALAS
router.get(
  '/ventas/sucursales/:sucursalId/salas',
  authenticateJWTOrServiceKey,
  async (req, res) => {
    try {
      const { data } = await axios.get(
        `${POS_API_BASE_URL}/api/salas/v1/sucursal/${req.params.sucursalId}`,
        { headers: { Authorization: `Bearer ${POS_API_TOKEN}` } }
      );
      res.json(data);
    } catch (error) {
...
// PASO 2: MESAS
router.get(
  '/ventas/sucursales/:sucursalId/mesas',
  authenticateJWTOrServiceKey,
  async (req, res) => {
    try {
      const { data } = await axios.get(
        `${POS_API_BASE_URL}/api/mesas/v1/sucursal/${req.params.sucursalId}`,
        { headers: { Authorization: `Bearer ${POS_API_TOKEN}` } }
      );
      res.json(data);
    } catch (error) {
...
// PASO 3: MENÚS
router.get(
  '/ventas/sucursales/:sucursalId/menus',
  authenticateJWTOrServiceKey,
  async (req, res) => {
    try {
      const { data } = await axios.get(
        `${POS_API_BASE_URL}/api/sucursales/v1/${req.params.sucursalId}/menus`,
        { headers: { Authorization: `Bearer ${POS_API_TOKEN}` } }
      );
      res.json(data);
    } catch (error) {
...
router.put('/ventas/menus/:id', authenticateJWTOrServiceKey, async (req, res) => {
  try {
    const { data } = await axios.put(
      `${POS_API_BASE_URL}/api/menus/v1/${req.params.id}`,
      req.body,
      { headers: { Authorization: `Bearer ${POS_API_TOKEN}`, 'Content-Type': 'application/json' } }
    );
    res.json(data);
...
// PASO 3: MENU ITEMS
router.get(
  '/ventas/sucursales/:sucursalId/menu-items',
  authenticateJWTOrServiceKey,
  async (req, res) => {
    try {
      const { data } = await axios.get(
        `${POS_API_BASE_URL}/api/menuitems/v1/sucursal/${req.params.sucursalId}`,
        { headers: { Authorization: `Bearer ${POS_API_TOKEN}` } }
      );
      res.json(data);
    } catch (error) {
...
router.put('/ventas/menu-items/:id', authenticateJWTOrServiceKey, async (req, res) => {
  try {
    const { data } = await axios.put(
      `${POS_API_BASE_URL}/api/menuitems/v1/${req.params.id}`,
      req.body,
      { headers: { Authorization: `Bearer ${POS_API_TOKEN}`, 'Content-Type': 'application/json' } }
    );
    res.json(data);
...
router.post('/ventas/menus/:menuId/items', authenticateJWTOrServiceKey, async (req, res) => {
  try {
    const { data } = await axios.post(
      `${POS_API_BASE_URL}/api/menus/v1/${req.params.menuId}/menu-items`,
      req.body,
      { headers: { Authorization: `Bearer ${POS_API_TOKEN}`, 'Content-Type': 'application/json' } }
    );
    res.json(data);
